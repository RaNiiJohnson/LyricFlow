...
export function SongCard({ song, className }: SongCardProps) {
  ...
  return (
    <Link href={`/lyrics/${song.id}`} className={`block ${className}`}>
      <Card className="song-card group overflow-hidden bg-card/50 border-border hover:border-accent/50 transition-all duration-300 ease-in-out cursor-pointer p-0">
        {/* Thumbnail */}
